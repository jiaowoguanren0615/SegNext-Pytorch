...
class AdjustGamma:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        return F.adjust_gamma(img, self.gamma, self.gain), mask
...
class RandomResizedCrop:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        img = F.resize(img, (nH, nW), F.InterpolationMode.BILINEAR)
        mask = F.resize(mask, (nH, nW), F.InterpolationMode.NEAREST)
        ...
        if img.shape[1:] != self.size:
            ...
            mask = F.pad(mask, padding, fill=self.seg_fill)
        return img, mask
...
class RandomAdjustSharpness:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        if random.random() < self.p:
            img = F.adjust_sharpness(img, self.sharpness)
        return img, mask
...
class RandomGaussianBlur:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        if random.random() < self.p:
            img = F.gaussian_blur(img, self.kernel_size)
        return img, mask
...
class RandomGrayscale:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        if random.random() < self.p:
            img = F.rgb_to_grayscale(img, 3)
        return img, mask


class Equalize:
    def __call__(self, image, label):
        return F.equalize(image), label
...
class Posterize:
    ...
    def __call__(self, image, label):
        return F.posterize(image, self.bits), label
